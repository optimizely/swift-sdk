...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ application: UIApplication) {
        ...
        if #available(iOS 13.0, *) {
            BGTaskScheduler.shared.register(forTaskWithIdentifier: fetchTaskId, using: nil) { task in
                OptimizelyBackgroundManager.handleBackgroundDatafileFetchTask(task: task as! BGAppRefreshTask)
            }
        }
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        if #available(iOS 13.0, *) {
            OptimizelyBackgroundManager.scheduleBackgroundDatafileFetch()
        }
    }
