...
extension OptimizelyError: CustomStringConvertible {
    ...
    var reason: String {
        ...
        case .evaluateAttributeInvalidType(let hint):       message = "Evaluation condition has an invalid value (\(hint))"
        case .evaluateAttributeValueOutOfRange(let hint):   message = "Evaluation condition has a value out of range (\(hint))"
        case .evaluateAttributeInvalidFormat(let hint):     message = "Evaluation condition has an invalid format (\(hint))"
