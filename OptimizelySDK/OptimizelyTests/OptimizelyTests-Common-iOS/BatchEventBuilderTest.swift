...
//  OptimizelySwiftSDK
//
//  Created by Thomas Zurkan on 2/27/19.
...
let kDatafileNameAnonymizeIPFalse = "test_data_25_experiments";
let kUserId = "6369992312";
let kAccountId = "6365361536";
let kProjectId = "6377970066";
let kRevision = "83";
let kLayerId = "1234";
let kEventRevenue = 88;
let kEventValue = 123.456;
let kTotalRevenueId = "6316734272";
let kAttributeKeyBrowserType = "browser_type";
let kAttributeValueFirefox = "firefox";
...
let kEventWithoutAudienceName = "testEvent";
var eventWithoutAudience:Event?
let kEventWithoutAudienceId = "6372590948";
let kExperimentWithoutAudienceKey = "testExperiment1";
let kExperimentWithoutAudienceId = "6367863211";
let kVariationWithoutAudienceId = "6384330452";

// events with experiment and audiences
let kEventWithAudienceName = "testEventWithAudiences";
var eventWithAudience:Event?
let kEventWithAudienceId = "6384781388";
let kExperimentWithAudienceKey = "testExperimentWithFirefoxAudience";
var experimentWithAudience:Experiment?
let kExperimentWithAudienceId = "6383811281";
let kVariationWithAudienceId = "6333082303";

// experiment not running parameters
let kEventWithExperimentNotRunningName = "testEventWithExperimentNotRunning";
let kEventWithExperimentNotRunningId = "6380961307";
let kExperimentNotRunningKey = "testExperimentNotRunning";
let kExperimentNotRunningId = "6367444440";

// events without experiments
let kEventWithoutExperimentName = "testEventWithoutExperiments";
let kEventWithoutExperimentId = "6386521015";

// events with multiple experiments
let kEventWithMultipleExperimentsName = "testEventWithMultipleExperiments";
let kEventWithMultipleExperimentsId = "6372952486";

// events with invalid experiments
let kEventWithInvalidExperimentName = "testEventWithInvalidExperiment";
let kEventWithInvalidExperimentId = "6370392433";
...
class BatchEventBuilderTest : XCTestCase {
    ...
    var  eventBuilder:BatchEventBuilder?
    var  bucketer:OPTBucketer?;
    var  begTimestamp:Date?
    var  attributes:[String: Any]?
    var  attributeIds:[String: Any]?
    var reservedAttributes:[String:Any]?

    override func setUp() {
        super.setUp()
        let datafile = loadJSONDatafileIntoDataObject(datafile: kDatafileName)
        self.optimizely = OptimizelyManager(sdkKey: "", periodicDownloadInterval: nil)
        do {
        ...
        self.attributes = [kAttributeKeyBrowserType : kAttributeValueFirefox,
            kAttributeKeyBrowserVersion : 68.1,
            kAttributeKeyBrowserBuildNumber : 106,
            kAttributeKeyBrowserIsDefault : true
        ]
        self.attributeIds = [kAttributeKeyBrowserType : "6359881003",
            kAttributeKeyBrowserVersion : "6359881004",
            kAttributeKeyBrowserBuildNumber : "6359881005",
            kAttributeKeyBrowserIsDefault : "6359881006"
        ]
        self.reservedAttributes = [Constants.Attributes.OptimizelyBotFilteringAttribute : true];
        eventWithoutAudience = self.optimizely?.config.project.events.filter({$0.key == kEventWithoutAudienceName }).first
        eventWithAudience = self.optimizely?.config.project.events.filter({$0.key == kEventWithAudienceName }).first
        experimentWithAudience = self.optimizely?.config.project.experiments.filter({$0.key == kExperimentWithAudienceKey }).first
    
        // need to do this cast because this is what happens when we get the event time stamp
        let currentTimeInterval = Date.timeIntervalBetween1970AndReferenceDate
        let currentTimeIntervalCast = currentTimeInterval
        self.begTimestamp = Date(timeIntervalSince1970: currentTimeInterval)
    }
 ...
 func testBuildConversionTicketWithNoAudience() {
    let data = BatchEventBuilder.createConversionEvent(config: (self.optimizely?.config)!, decisionService: (self.optimizely?.decisionService)!, eventKey:kEventWithoutAudienceName , userId: kUserId, attributes: nil, eventTags: nil)
    print(data)
