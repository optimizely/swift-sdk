...
class DecisionServiceTests_Others: XCTestCase {
    ...
    func testValidatePreconditions() {
        ...
        do {
            let isValid = try self.decisionService.isInExperiment(experiment: experiment,
                                                                  userId: self.kUserId,
    ...
    func testValidatePreconditionsBadAttributes() {
        ...
        let experiment = self.config.getExperiment(key: kExperimentWithAudienceKey)!
        do {
            _ = try self.decisionService.isInExperiment(experiment: experiment,
                                                        userId: self.kUserId,
    ...
    func testUserInExperimentWithEmptyAudienceIdAndConditions() {
        ...
        let isValid = try? (optimizely.decisionService as! DefaultDecisionService).isInExperiment(experiment: experiment,
                                                                                                  userId: kUserId,
                                                                                                  attributes: self.attributes)
        XCTAssert(isValid!)
    }
    ...
    func testGetVariationExperimentNotRunning() {
        ...
        let variation = self.decisionService.getVariation(userId: kUserId,
    ...
    func testGetVariationWithWhitelistedVariation() {
        ...
        let variation = self.decisionService.getVariation(userId: kWhitelistedUserId_test_data_10_experiments,
