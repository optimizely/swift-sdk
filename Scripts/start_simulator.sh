...
if [ "$SIMULATOR_XCODE" != 12.4 ]; then
    ...
    then
        os_folder="${os_folder}.platform/Developer"
    else
        os_folder="${os_folder}.platform"
    fi

    # Create link and create simulators which are not part of the current xcode version
    sudo ln -s /Applications/Xcode_$SIMULATOR_XCODE.app/Contents/Developer/Platforms/$os_folder/Library/Developer/CoreSimulator/Profiles/Runtimes/$OS_TYPE.simruntime /Library/Developer/CoreSimulator/Profiles/Runtimes/$OS_TYPE\ $OS.simruntime
