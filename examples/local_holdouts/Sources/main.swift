/*
 Local Holdouts Bug Bash -- Swift SDK (command-line version)
 ...
 HOW TO RUN:
   ...
   vim Sources/main.swift   (set SDK_KEY on line 22)
   swift run
*/
...
func inspectProject(_ client: OptimizelyClient) {
    print("\n============================================================")
    ...
    for (fk, feat) in optConfig.featuresMap.sorted(by: { $0.key < $1.key }) {
        print("    \(fk):")
...
guard SDK_KEY != "YOUR_SDK_KEY_HERE" else {
    print("ERROR: Set your SDK key in the SDK_KEY constant at the top of main.swift")
    exit(1)
}
...
let d = user.decide(key: FLAG1, options: [.includeReasons])
printDecision(label: "user_123 on \(FLAG1)", decision: d)
...
// let d3 = userNoAttr.decide(key: FLAG1)
...
// print("\n--- DecideForKeys ---")
// let uc = optimizely.createUserContext(userId: "user_123")
// let subset = uc.decide(keys: [FLAG1, FLAG2])
// for (fk, d) in subset {
...
// let uc = optimizely.createUserContext(userId: "user_123")
// let d1 = uc.decide(key: FLAG1)                                           // fires impression
// let d2 = uc.decide(key: FLAG1, options: [.disableDecisionEvent])          // no impression
...
//     let uc = optimizely.createUserContext(userId: uid)
//     let d = uc.decide(key: FLAG1)
//     let tag = isHoldout(d) ? "HOLDOUT" : "normal"
